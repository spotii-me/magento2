...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    */
    public function setInventoryBefore($quote) {
       ...
       try{
       $this->logSpotiiActions("before 1");
       $orderData =$quote->getAllVisibleItems();
       $this->logSpotiiActions("before 2");
       $this->logSpotiiActions(sizeof($orderData));
       foreach($orderData as $item){
         $product = $this->_productFactory->create()->load($item['product_id']);
         $this->logSpotiiActions($item['product_id']. "  " .intval($item['qty']));
        }
        ...
        $this->logSpotiiActions("in Data ID" .$increment_id);
